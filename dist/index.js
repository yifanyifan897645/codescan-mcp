...
async function walkDir(dir, maxFiles = 5000) {
    ...
    async function walk(current, depth) {
        if (depth > 20 || files.length >= maxFiles)
            return;
        let entries;
        try {
...
server.tool("scan_todos", "Find all TODO, FIXME, HACK, XXX, BUG, and OPTIMIZE comments in the codebase with file, line, and context", {
...
server.tool("project_stats", "Get codebase statistics: files by language, line counts, largest files, directory breakdown", {
    directory: z.string().describe("Absolute path to the project directory"),
}, async ({ directory }) => {
    const files = await walkDir(directory);
    ...
    for (const f of files) {
        if (!byExt[f.ext])
            byExt[f.ext] = { files: 0, lines: 0, size: 0 };
        byExt[f.ext].files++;
    ...
    const output = {
        ...
        byExtension: Object.fromEntries(Object.entries(byExt).sort((a, b) => b[1].lines - a[1].lines)),
        byDirectory: Object.fromEntries(Object.entries(byDir).sort((a, b) => b[1].lines - a[1].lines).slice(0, 20)),
        largestFiles: largest.map(f => ({ path: f.path, lines: f.lines, sizeKB: Math.round(f.size / 1024) })),
    };
    return { content: [{ type: "text", text: JSON.stringify(output, null, 2) }] };
});
// Tool 3: Find long/complex files
server.tool("find_complex_files", "Find files that are unusually long, deeply nested, or have high complexity indicators", {
    directory: z.string().describe("Absolute path to the project directory"),
    ...
    const results = [];
    for (const f of files) {
        if (f.lines < lineThreshold && f.path.split("/").length <= 5)
            continue;
        const flags = [];
        if (f.lines >= lineThreshold)
            flags.push(`${f.lines} lines (threshold: ${lineThreshold})`);
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (indent > maxIndent && indent < 100)
                    maxIndent = indent;
            ...
            if (maxIndent >= 20)
                flags.push(`deep nesting (max indent: ${maxIndent} chars)`);
            if (longFunctions > 0)
                flags.push(`${longFunctions} long function(s) (>50 lines)`);
            if (flags.length > 0) {
                results.push({
                    path: f.path,
                    lines: f.lines,
...
server.tool("check_dependencies", "Analyze project dependencies: count, outdated signals, duplicate lockfiles, missing lockfiles", {
    directory: z.string().describe("Absolute path to the project directory"),
}, async ({ directory }) => {
    ...
    try {
        ...
        try {
            await stat(join(directory, "yarn.lock"));
            hasLockfile = true;
        }
        catch { }
        try {
            await stat(join(directory, "pnpm-lock.yaml"));
            hasLockfile = true;
        }
        catch { }
        try {
            await stat(join(directory, "bun.lockb"));
            hasLockfile = true;
        }
        catch { }
...
server.tool("health_report", "Generate a full codebase health report: stats, TODOs, complexity, dependencies, and a letter grade", {
    directory: z.string().describe("Absolute path to the project directory"),
}, async ({ directory }) => {
    const files = await walkDir(directory);
    const totalLines = files.reduce((s, f) => s + f.lines, 0);
    ...
    if (todoCount > 10)
        report.recommendations.push(`Address ${todoCount} TODO/FIXME comments — prioritize FIXMEs`);
    if (largeFiles > 3)
        report.recommendations.push(`${largeFiles} files exceed 300 lines — consider splitting`);
    if (avgLines > 150)
        report.recommendations.push(`Average file length (${avgLines}) is high — aim for <150`);
    if (report.recommendations.length === 0)
        report.recommendations.push("Codebase looks healthy!");
